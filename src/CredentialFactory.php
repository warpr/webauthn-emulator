...
class CredentialFactory
{
    public static function makeFromOptions(array $options): CredentialInterface
    {
        ...
        if (
            empty($options['pubKeyCredParams']) ||
            !in_array(['alg' => -7, 'type' => 'public-key'], $options['pubKeyCredParams'])
        ) {
            throw new InvalidArgumentException(
                'Requested pubKeyCredParams does not contain supported type. Only ES256 (alg: -7) is supported at the moment.'
            );
        }
